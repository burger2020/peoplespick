...
import picturePick.picture.main_user.profile_rating.Dialog.Edit_Photo_Add_Dialog
import picturePick.picture.main_user.profile_rating.R
import picturePick.picture.main_user.profile_rating.data.pic_basic_data.Companion.pick_photo_num
...
class Edit_adapter_gallary(val context:Context, val photo_url:ArrayList<Uri>, val check_:ArrayList<Boolean>,
    ...
    companion object {
        var click_photo_bitmap : ArrayList<Bitmap> = arrayListOf()
    ...
    override fun getView(position: Int, view: View?, parent: ViewGroup?): View? {
        ...
        if(mainView == null){
            ...
            holder.imageView?.setOnClickListener({
                    if(check_[position]) { // 체크 해제(체크 돼있을시)
                        ...
                        check_[position] = false
                        click_photo_uri.remove(photo_url[position])
                        click_photo_position.remove(position)
                    }
                    else if(!check_[position]&& pick_photo_num>now_pickPhoto_num){  // 체크  (체그 안되어있을시 && 최대 선택 이하)
                        now_pickPhoto_num++
                        ...
                        check_[position] = true
                    }else if(!check_[position]&&pick_photo_num<=now_pickPhoto_num){
        ...
        if(check_[position])
            ...
            holder.checkBox?.setBackgroundResource(R.mipmap.ic_edit_checkbox_nonselect)
        Glide.with(context).load(photo_url[position].toString()).into(holder.imageView)
